...
class ZeroFrictionManager:
    ...
    def _skill_root(self) -> Path:
        repository_skill = self.project_root / "skills" / "syntavra"
        ...
        if not (bundled / "SKILL.md").is_file():
            raise FileNotFoundError("Syntavra bundled skill is unavailable")
        return bundled
    ...
    def doctor(self) -> dict[str, Any]:
        ...
        if not installed:
            warnings.append({"code": "not-installed", "repair": "syntavra setup --apply"})
        elif not all(product_files.values()):
            warnings.append({"code": "product-bundle-incomplete", "repair": "syntavra repair --apply"})
        failed_hosts = [row for row in host_verification if not row.get("ok")]
        if failed_hosts:
            blocking.append({"code": "host-integration-verification-failed", "repair": "syntavra repair --apply"})
