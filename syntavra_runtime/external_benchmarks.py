...
class ExternalBenchmarkGate:
    ...
    def evaluate(cls, receipts: Iterable[ExternalBenchmarkReceipt], *, suite_id: str | None = None) -> dict[str, Any]:
        ...
        for pair in pairs:
            ...
            parity = (
                baseline.dataset_version == syntavra.dataset_version
                and baseline.harness_commit == syntavra.harness_commit
                and baseline.verifier_commit == syntavra.verifier_commit
                and baseline.environment_image_digest == syntavra.environment_image_digest
                and baseline.repository_commit == syntavra.repository_commit
                and baseline.provider == syntavra.provider
                and baseline.model == syntavra.model
                and baseline.model_config_hash == syntavra.model_config_hash
            )
            if not parity:
            ...
            if baseline.total_billable_tokens > 0:
                token_ratios.append(syntavra.total_billable_tokens / baseline.total_billable_tokens)
...
class ExternalSuiteRunner:
    """Bind an external benchmark harness to Syntavra's secure arm runner."""
