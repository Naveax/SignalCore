...
def _build_registry() -> MCPToolRegistry:
    ...
    registry.register(ToolDefinition(
        "syntavra.pipeline.describe", "Describe the canonical request/output pipeline", _object_schema(),
        lambda server, _: server.v6_pipeline.describe(), permissions=("evidence-read",),
    ))
    registry.register(ToolDefinition(
        "syntavra.config.effective", "Read effective canonical configuration and provenance", _object_schema(),
        lambda server, _: server.v6_config.load().to_dict(), permissions=("admin",),
    ))
    registry.register(ToolDefinition(
        "syntavra.evidence.stats", "Inspect encrypted evidence lifecycle statistics", _object_schema(),
        lambda server, _: server.evidence.stats(), permissions=("evidence-read",),
    ))
    registry.register(ToolDefinition(
        "syntavra.evidence.gc", "Plan or apply evidence retention garbage collection",
        _object_schema({
    ...
    registry.register(ToolDefinition(
        "syntavra.observability.metrics", "Read local structured metrics", _object_schema({"format": {"type": "string"}}),
    ...
    registry.register(ToolDefinition(
        "syntavra.plugins.list", "List permissioned plugins and quarantine state", _object_schema(),
        lambda server, _: server.v6_plugins.records(), permissions=("admin",),
    ))
    registry.register(ToolDefinition(
        "syntavra.scheduler.stats", "Inspect durable job scheduler queue and dead-letter state", _object_schema(),
        lambda server, _: server.v6_scheduler.stats(), permissions=("admin",),
    ))
    registry.register(ToolDefinition(
        "syntavra.backup.create", "Create an encrypted point-in-time state backup",
    ...
    registry.register(ToolDefinition(
        "syntavra.backup.verify", "Verify an encrypted state backup without restoring it",
...
def install() -> None:
    ...
    from .provider_extension import install as install_provider
    from .sandbox_extension import install as install_sandbox

    install_provider()
    install_sandbox()
    if getattr(MCPServer, "_syntavra_v001_unified_extension", False):
        return
    ...
    def extended_init(self: Any, *args: Any, **kwargs: Any) -> None:
        ...
        self.v6_scheduler = DurableJobScheduler(self.state_root / "scheduler.sqlite3")
        rollout_key = os.environ.get("SYNTAVRA_POLICY_SIGNING_KEY", "").encode("utf-8") or None
    ...
    def extended_exposed(self: Any) -> list[dict[str, Any]]:
        if getattr(type(self), "_syntavra_native_mcp_pipeline", False):
            return list(original_exposed(self))
    ...
    MCPServer._syntavra_v001_unified_extension = True
