...
class HostInstallationManager:
    """Atomic, reversible installer for SignalCore host integrations.
    ...
    def __init__(
        ...
        *,
        ...
        if not (self.skill_root / "SKILL.md").is_file():
            raise FileNotFoundError(f"SignalCore skill source is incomplete: {self.skill_root}")
    ...
    def _copy_tree_atomic(source: Path, target: Path) -> None:
        ...
        temporary = target.parent / f".{target.name}.signalcore-{uuid.uuid4().hex}"
        ...
        if spec.config_path:
            ...
            if not target.is_file():
                ...
                try:
                    ...
                    signalcore = (config.get("mcpServers") or {}).get("signalcore") if isinstance(config, Mapping) else None
                    if not isinstance(signalcore, Mapping) or signalcore.get("command") != "signalcore":
                        reasons.append("missing-signalcore-mcp")
