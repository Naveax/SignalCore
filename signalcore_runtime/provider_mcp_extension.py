...
def provider_tools() -> list[dict[str, Any]]:
    return [
        ...
        _tool("signalcore.provider.stats", "Inspect provider gateway request and replay statistics"),
        _tool("signalcore.provider.verify", "Verify provider request and response evidence integrity"),
    ]
...
def install() -> None:
    ...
    if getattr(MCPServer, "_signalcore_provider_extension_v4", False):
        return
    ...
    def extended_call_tool(self: Any, name: str, arguments: dict[str, Any]) -> Any:
        if name == "signalcore.provider.capabilities":
            return self.provider_gateway.capabilities(arguments.get("provider"))
        if name == "signalcore.provider.prepare":
        ...
        if name == "signalcore.provider.capture":
        ...
        if name == "signalcore.provider.replay":
        ...
        if name == "signalcore.provider.stats":
            return self.provider_gateway.stats()
        if name == "signalcore.provider.verify":
    ...
    MCPServer._signalcore_provider_extension_v4 = True
