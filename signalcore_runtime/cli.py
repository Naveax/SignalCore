...
def skill_root() -> Path:
    repository_skill = root() / "skills" / "signal-core"
...
def state_root(args: argparse.Namespace) -> Path:
    return Path(args.state_root).resolve(strict=False) if args.state_root else project(args, strict=False) / ".signalcore" / "runtime-v3"
...
def command_benchmark(args: argparse.Namespace) -> int:
    if args.action == "generate-config":
        ...
        result = compare_results(load_arm_results(Path(args.baseline)), load_arm_results(Path(args.signalcore)), tier=args.tier, config=config)
        if args.output:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="signalcore", description="SignalCore 0.0.1 unified runtime control plane")
    ...
    bc = bs.add_parser("compare"); bc.add_argument("--baseline", required=True); bc.add_argument("--signalcore", required=True); bc.add_argument("--config", required=True); bc.add_argument("--tier", required=True); bc.add_argument("--output")
