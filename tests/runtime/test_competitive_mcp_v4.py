...
class CompetitiveMCPV4Tests(unittest.TestCase):
    ...
    def test_full_profile_exposes_combined_competitive_surface(self):
        with patch.dict(os.environ, {"SIGNALCORE_MCP_PROFILE": "full"}, clear=False):
        ...
        required = {
            ...
            "signalcore.fabric.compact",
            "signalcore.fabric.cache_align",
            "signalcore.fabric.platform_plan",
    ...
    def test_optimized_profile_reduces_manifest_but_keeps_core(self):
        ...
        self.assertIn("signalcore.fabric.route", names)

    def test_fabric_calls_route_compact_cache_and_platform(self):
        route = self.server.call_tool("signalcore.fabric.route", {"command": ["pytest", "-q"]})
        ...
        plan = self.server.call_tool("signalcore.fabric.platform_plan", {"host": "claude-code"})
        self.assertTrue(plan["enforced"])

    def test_exact_range_and_session_dag_tools(self):
        ranged = self.server.call_tool(
            "signalcore.inspect.range", {"path": "sample.py", "start_line": 1, "end_line": 3}
        )
        self.assertIn("class Demo", ranged["text"])
        session = self.server.call_tool("signalcore.session.open", {"session_id": "mcp-v4"})
        ...
        compact = self.server.call_tool(
            "signalcore.session.compact", {"session_id": session["session_id"], "leaf_size": 2, "fanout": 2}
        )
        expanded = self.server.call_tool("signalcore.session.expand", {"summary_id": compact["summary_id"]})
        self.assertEqual(expanded["coverage"], 8)
        verified = self.server.call_tool("signalcore.session.verify", {"session_id": session["session_id"]})
